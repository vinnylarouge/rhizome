...
| 17 | Paper genre (`/compile`) | Chatham House-style roundtable report | Matches the actual event (anonymised, exec-summary → findings → tensions → recommendations → open questions). Grammar extracted from 8 verified exemplars (`docs/paper-grammar.md`). |
| 18 | Grammar/style provenance | Researched once, baked into `docs/` as files; runtime only *fills* them | Reproducible, hand-editable control surface; re-deriving per run would be slow and non-deterministic (mirrors the vendored heuristic docs). |
| 19 | Citation receipts | GPT-5.5 + Responses API `web_search`, then a **strict single-verifier** (fetch the URL, quote the supporting passage, or drop & flag `[unsupported]`) | Real, grounded URLs server-side; structural anti-hallucination (a cite cannot exist without a fetched supporting quote). Conservative scope: empirical claims + heuristic provenance only. Audit trail in `receipts.md`. |
| 20 | Paper model | `LOOM_PAPER_MODEL=gpt-5.5` (Responses API); live workers stay on `gpt-5.4` | Only path to grounded URL receipts without a 3rd-party search API; smoke-tested against the real key. Verifier has two modes (evidence vs provenance) — one prompt wrongly rejected provenance sources. |
| 21 | Bibliography | biblatex `numeric` + author values wrapped in `{{double braces}}` | Web-extracted author lists are comma-separated and trip biber's "too many commas" name parser; double-braces protect them as literals; numeric suits sources lacking reliable per-author years. |
| 22 | Writing style | Anti-"AI-tell" guide injected into every prompt + final smell-check rewrite pass | Sourced tells (`docs/style-guide.md`); the signal is stylistic not topical, so concreteness + commitment + uneven rhythm beat detectors. Smell-check operates on plain prose (never mangles LaTeX/cites). |
| 23 | Parametric genres | One pipeline, three genres — policy-brief (default) / roundtable / academic — selected by `/compile [genre]` | Same graph material, different section order + voice + template (`src/paper/genres.js`); a single roundtable read too dry/narrow for executives who wanted a brief. |
| 24 | Typeset look | pdflatex + TeX Gyre packages (tgpagella/tgheros/tgtermes), not xelatex+fontspec | Real book faces over Computer Modern without xelatex's OS-font-discovery fragility; the policy brief uses `tcolorbox` callouts for Key Findings + Recommendations. |
| 25 | Prose register (revised) | Split "ban the tells" (kept strict) from "be terse/bursty" (dropped); smell-check now preserves length + warmth | The first anti-AI-tell rules over-corrected into clipped, dry prose, and the smell-check re-flattened it. Also: every `json_object` prompt must contain the word "json" (now guarded in `chatJSON`) or it 400s and silently drops a section. |
